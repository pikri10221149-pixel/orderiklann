...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['barang'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE barang SET id_kategori=?, nama_barang=?, merk=?,ukuran=?,materi=?, 
				harga_beli=?, harga_jual=?, diskon=?,ppn=?, satuan_barang=?, stok=?, tgl_update=?  WHERE id_barang=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
